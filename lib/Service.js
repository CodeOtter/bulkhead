var Response = require('./Response'),
	...
	di = require('./DependencyInjection'),
	async = require('async');
...
module.exports = function(modelName, criteriaMapping) {
	...
	this.getModel = function() {
		if(!this.model) {
			...
			if(this.plugin.models[name]) {
				this.model = this.plugin.models[name];
			}
		}
	 ...
	 * @param	Function	The callback to fire when the search is finished.
	 */
	this.findEach = function(criteria, iterate, done) {
		if(sails) {
			sails.log.silly('findAndDo will be deprecated as of 0.0.8.  Please use findEach instead.');
		}
		var self = this;
