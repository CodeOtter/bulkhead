...
function loadPackage(sails, location, section, options, done) {
	...
	var plugin = Plugin(location + '/api/services/');

	buildDictionary.optional(options, function(err, modules) {
		if(err) return done(err);
		if(section == 'config') {
			for(var i in modules) {
				plugin.config = _.extend(plugin.config || {}, modules[i]);	
			}
		} else {
			plugin[section] = _.extend(plugin[section] || {}, modules);
		}
...
module.exports = {
	...
	load: function(sails, done) {
		...
		for(var i in global._Bulkhead.paths) {
			var pkg = global._Bulkhead.paths[i];
		...
		async.parallel(packages, function() {
			// We no longer register globals due to namespacing violations
			sails.plugins = Plugin;

			if (sails.config.globals.services) {
				_.each(global._Bulkhead.plugins, function(module) {
					_.each(module, function(version) {
						_.each(version.services, function(service, identity) {
							version[service.globalId || service.identity] = service;	
						});
					});
				});
			}
			...
			if (sails.config.globals && sails.config.globals.models) {
				_.each(global._Bulkhead.plugins, function(module) {
					_.each(module, function(version) {
						_.each(version.models, function(model, identity) {
							version[model.globalId || model.identity] = model;
						});
					});
				});
			}
	...
	add: function() {
		...
		if(pos > -1) {
			...
			if(global._Bulkhead.paths.indexOf(name) == -1) {
				global._Bulkhead.paths.push(name);
				// Register the package as a plugin
				return Plugin(path);
			}
		}
		return false;
