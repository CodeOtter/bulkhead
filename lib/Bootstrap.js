...
function loadPackage(sails, location, section, options, done) {
	...
	var plugin = Plugin(path.basename(location));
...
module.exports = {
	...
	load: function(sails, done) {
		...
		for(var i in global.Bulkhead.paths) {
			var pkg = global.Bulkhead.paths[i];
	...
	add: function() {
		...
		if(pos > -1) {
			...
			if(Bulkhead.paths.indexOf(name) == -1) {
				Bulkhead.paths.push(name);
